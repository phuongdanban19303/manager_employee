...
import { FaArrowUp, FaChartLine, FaChevronDown, FaHistory, FaInfoCircle, FaLightbulb, FaPaperPlane, FaSave, FaSearch, FaUserTie } from 'react-icons/fa';
...
const EventManagement = () => {
  ...
  const { activeEmployees, selectedEmployee, submitting, loading: empLoading, isSaved, currentFormId, editingRecordId } = useSelector(state => state.event);
  ...
  const [formData, setFormData] = useState({
    salary: { times: 1, oldLevel: '', newLevel: '', reason: '' ,receiverId:'null'},
  ...
  const handleOpenSubmit = () => {
    ...
    if (!isSaved || !currentFormId) return alert("Vui lòng Lưu bản ghi trước khi trình duyệt!");
    ...
    <div className="animate-fadeIn pb-24">
      ...
      <div className="mb-12">
        ...
        <div className="flex items-center gap-2 mt-2">
            ...
            <p className="text-slate-400 font-bold text-[10px] uppercase tracking-widest">Quy trình điều chuyển & Thăng tiến tập trung</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-[3rem] shadow-sm border border-slate-100 overflow-hidden mb-12">
        <div className="p-10 border-b border-slate-50 flex flex-col md:flex-row items-center justify-between gap-6">
           <h3 className="text-xl font-black text-slate-800 uppercase tracking-tighter">Chọn nhân viên xử lý</h3>
           ...
           <table className="w-full text-left">
              <thead>
                <tr className="bg-slate-50/50">
                  ...
                  <th className="px-10 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Team</th>
                  <th className="px-10 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">Hành động</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50">
                {filteredEmployees.map(emp => (
                  <tr key={emp.id} className={`hover:bg-blue-50/30 transition-all ${selectedEmployee?.id === emp.id ? 'bg-blue-50/50' : ''}`}>
                    ...
                    <td className="px-10 py-4">
                       <span className="text-[10px] font-black text-slate-400 uppercase">#{emp.team}</span>
                    </td>
                    <td className="px-10 py-4 text-right">
                       <button onClick={() => handleSelectEmployee(emp)} className="px-6 py-2 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase transition-all active:scale-95 shadow-lg shadow-slate-200">Chọn xử lý</button>
                    </td>
                  </tr>
                ))}
              </tbody>
           ...
           <div className="bg-white rounded-[3.5rem] shadow-2xl border border-slate-100 overflow-hidden">
              <div className="px-12 py-8 bg-slate-900 text-white flex items-center justify-between">
                 <div className="flex items-center gap-6">
                    <div className="w-12 h-12 bg-white/10 rounded-xl flex items-center justify-center text-blue-400"><FaUserTie /></div>
                    <p className="text-lg font-black uppercase">Thiết lập diễn biến: {selectedEmployee.fullName}</p>
                 </div>
                 <div className="flex items-center gap-4">
                    {editingRecordId && <span className="px-4 py-1.5 bg-amber-500 text-white text-[9px] font-black rounded-lg uppercase tracking-widest">Đang chỉnh sửa bản ghi #{editingRecordId}</span>}
                    <button onClick={() => dispatch(setSelectedEmployee(null))} className="text-white/50 hover:text-white transition-colors"><FaChevronDown /></button>
                 </div>
              </div>
                 ...
                 <div className="max-w-5xl mx-auto space-y-10">
                        ...
                        <div className="grid grid-cols-2 gap-10">
                            ...
                            <div className="grid grid-cols-2 gap-6">
                                <div className="space-y-3">
                                    ...
                                    <input value={formData.salary.oldLevel} onChange={e => handleInputChange('salary', 'oldLevel', e.target.value)} className="w-full px-6 py-4 bg-slate-50 border-0 rounded-2xl font-bold text-slate-700"/>
                                </div>
                                <div className="space-y-3">
                                    <label className="text-[10px] font-black text-slate-400 uppercase">Bậc lương mới *</label>
                                    <input value={formData.salary.newLevel} onChange={e => handleInputChange('salary', 'newLevel', e.target.value)} className="w-full px-6 py-4 bg-slate-50 border-2 border-blue-100 rounded-2xl font-bold text-slate-700 focus:border-blue-600 outline-none"/>
                                </div>
                            </div>
                    ...
                    {activeTab === 'promotion' && (
                        <div className="grid grid-cols-2 gap-10">
                            ...
                            <div className="space-y-3">
                                <label className="text-[10px] font-black text-slate-400 uppercase">Chức vụ bổ nhiệm *</label>
                                <input value={formData.promotion.newPosition} onChange={e => handleInputChange('promotion', 'newPosition', e.target.value)} className="w-full px-6 py-4 bg-slate-50 border-2 border-purple-100 rounded-2xl font-bold text-slate-700 focus:border-purple-600 outline-none"/>
                            </div>
                            <div className="col-span-2 space-y-3">
                    ...
                    {activeTab === 'proposal' && (
                        <div className="space-y-10">
                            <div className="space-y-3">
                                <label className="text-[10px] font-black text-slate-400 uppercase">Nội dung đề xuất *</label>
                                <input value={formData.proposal.content} onChange={e => handleInputChange('proposal', 'content', e.target.value)} className="w-full px-8 py-6 bg-white border-2 border-indigo-100 rounded-3xl font-black text-slate-800 text-xl tracking-tight outline-none focus:border-indigo-600 shadow-sm"/>
                            </div>
                            <div className="space-y-3">
                                <label className="text-[10px] font-black text-slate-400 uppercase">Chi tiết giải pháp</label>
              ...
              <div className="px-16 py-10 bg-slate-50/50 border-t flex items-center justify-between">
                 <p className="text-[10px] font-black text-slate-400 uppercase flex items-center gap-2 italic">
                    <FaInfoCircle /> {isSaved ? "Dữ liệu đã sẵn sàng trình lãnh đạo." : "Lưu nháp để lấy mã định danh trước khi trình."}
                 </p>
                 <div className="flex items-center gap-6">
                    <button 
                      onClick={handleSave}
                    ...
                    >
                       <FaSave /> {editingRecordId ? "CẬP NHẬT THAY ĐỔI" : "LƯU DIỄN BIẾN"}
                    </button>
                    <button 
           ...
           <div className="bg-white rounded-[3.5rem] shadow-sm border border-slate-100 overflow-hidden">
              <div className="px-12 py-10 border-b flex items-center justify-between bg-slate-50/30">
                 <div className="flex items-center gap-4">
                    ...
                    <h3 className="text-xl font-black text-slate-800 uppercase tracking-tight">Lịch sử {activeTab === 'salary' ? 'tăng lương' : activeTab === 'promotion' ? 'thăng chức' : 'đề xuất'}</h3>
                 </div>
              </div>
