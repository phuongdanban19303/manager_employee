...
import { FaTimes, FaCalendarAlt, FaFileAlt, FaUserTie, FaPaperPlane, FaSpinner, FaExclamationTriangle } from 'react-icons/fa';
import { fetchManagersRequest, submitTerminationFlowRequest } from '../../store/scile/terminationSlice';
...
const TerminationModal = ({ employee, onClose }) => {
  ...
  const handleSubmit = () => {
    ...
    dispatch(submitTerminationFlowRequest({
      ...
      <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden border border-slate-100">
        ...
        <div className="px-10 py-8 border-b bg-rose-50/30 flex items-center justify-between">
          <div className="flex items-center gap-4">
             <div className="w-12 h-12 bg-rose-600 rounded-2xl flex items-center justify-center text-white shadow-lg shadow-rose-200"><FaFileAlt size={20} /></div>
             <div>
                <h3 className="text-xl font-black text-slate-800 uppercase tracking-tight">Quyết định kết thúc</h3>
                <p className="text-[10px] text-rose-600 font-bold uppercase tracking-widest mt-1">Nhân viên: {employee.fullName || employee.name}</p>
             </div>
          </div>
        ...
        <div className="p-10 space-y-8">
          <div className="bg-amber-50 p-5 rounded-2xl border border-amber-100 flex gap-4">
             ...
             <p className="text-[10px] text-amber-700 font-bold leading-relaxed uppercase">
                Lưu ý: Hành động này sẽ trình lãnh đạo phê duyệt việc chấm dứt hồ sơ nhân sự của nhân viên này.
             </p>
          </div>
          ...
          <div className="grid grid-cols-1 gap-6">
             ...
             <div className="space-y-3">
                ...
                <select 
                    ...
                    onChange={e => setFormData(p => ({...p, receiverId: e.target.value}))}
                ...
                >
                    ...
                    {managers.map(m => (
                        <option key={m.id} value={m.id}>{m.fullname} ({m.team})</option>
                    ))}
                </select>
             </div>
             ...
             <div className="space-y-3">
                ...
                <textarea 
                    ...
                    onChange={e => setFormData(p => ({...p, terminationReason: e.target.value}))}
        ...
        <div className="px-10 py-8 bg-slate-50/50 border-t flex gap-4">
           <button onClick={onClose} className="flex-1 py-4 font-black text-slate-400 uppercase text-[10px] tracking-widest">Hủy bỏ</button>
           <button 
           ...
           >
              {submitting ? <FaSpinner className="animate-spin" /> : <FaPaperPlane />} TRÌNH LÃNH ĐẠO
           </button>
        </div>
      </div>
    </div>
