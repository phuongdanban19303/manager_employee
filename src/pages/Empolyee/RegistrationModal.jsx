...
import { createRegistrationRequest, fetchExistingRegistrationRequest, updateRegistrationRequest } from '../../store/scile/employeeSlice.js';
...
const RegistrationModal = ({ onClose }) => {
 const dispatch = useDispatch();
  ...
  // 1. Khi mở modal, check xem đã có form đăng ký nào chưa
  useEffect(() => {
  ...
  // 2. Nếu có form rồi, đổ dữ liệu vào state
  useEffect(() => {
  ...
  const handleSaveDoc = () => {
    ...
    // Nếu đã có currentFormId -> Chạy luồng UPDATE
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/70 backdrop-blur-sm p-6 animate-fadeIn">
      <div className="bg-white w-full max-w-7xl h-[90vh] rounded-[3rem] shadow-2xl flex overflow-hidden border border-white/20">
        ...
        <div className="w-80 bg-slate-50 border-r flex flex-col shrink-0">
          <div className="p-10 border-b bg-white">
            <h3 className="text-xl font-black text-slate-800 tracking-tight uppercase">Danh sách hồ sơ</h3>
        ...
        <div className="flex-1 flex flex-col bg-white overflow-hidden">
          <div className="px-12 py-10 border-b flex items-center justify-between">
            <div>
               ...
               <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1 flex items-center gap-2">
                 ĐANG CHỈNH SỬA: {docTypes.find(d => d.id === activeDoc)?.label}
                 {registration.currentFormId && <span className="bg-amber-100 text-amber-600 px-2 py-0.5 rounded text-[8px] tracking-normal font-black">CHẾ ĐỘ CẬP NHẬT (ID: {registration.currentFormId})</span>}
               </p>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-12 custom-scrollbar bg-[#fcfcfd]">
             {registration.loading && !registration.currentFormData ? (
               <div className="flex flex-col items-center justify-center h-full gap-4 text-slate-400">
                  ...
                  <p className="text-[10px] font-black uppercase tracking-widest">Đang kiểm tra lịch sử đăng ký...</p>
               </div>
             ) : (
               <div className="max-w-4xl mx-auto space-y-10">
                  ...
                  <div className="space-y-3">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Tóm tắt quá trình làm việc (Resume)</label>
                    <textarea 
                      name="resume"
                  ...
                  <div className="space-y-3">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Ghi chú cho lãnh đạo</label>
                    <textarea 
                      name="note"
                      value={formData.note}
                      onChange={handleChange}
          ...
          <div className="px-12 py-8 border-t bg-slate-50/50 flex items-center justify-between">
             <button onClick={onClose} className="px-8 py-3 font-black text-slate-400 hover:text-slate-600 transition-all uppercase text-[10px] tracking-[0.2em]">Hủy bỏ</button>
             <div className="flex items-center gap-5">
                ...
                >
                   {registration.loading ? <FaSpinner className="animate-spin" /> : <FaSave />} 
                   {registration.currentFormId ? 'CẬP NHẬT THÔNG TIN' : 'LƯU THÔNG TIN'}
                </button>
                <button 
                  ...
                  className={`flex items-center gap-3 px-10 py-4 font-black rounded-[1.25rem] transition-all active:scale-95 text-[10px] uppercase shadow-2xl ${
                    registration.currentFormId ? 'bg-blue-600 text-white shadow-blue-200 hover:bg-blue-700' : 'bg-slate-200 text-slate-400 cursor-not-allowed'
                  }`}
                >
      ...
      {showLeaderPopup && (
        <LeaderSubmitModal 
          ...
          onSuccess={() => { setShowLeaderPopup(false); onClose(); }}
        />
      )}
    </div>
  );
