...
import { FaUserTie, FaPen, FaCalendarAlt, FaPaperPlane, FaTimes, FaSpinner } from 'react-icons/fa';
import { fetchManagersRequest, submitToManagerRequest } from '../../store/scile/employeeSlice';
...
const LeaderSubmitModal = ({ onClose, onSuccess }) => {
  ...
  const selectedManager = registration.managers.find(m => m.id === submitData.receiverId);
  ...
  const handleSubmit = () => {
    ...
    dispatch(submitToManagerRequest({
      ...
      <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-[0_50px_100px_rgba(0,0,0,0.2)] overflow-hidden border border-slate-100">
        ...
        <div className="px-10 py-8 border-b bg-slate-50/50 flex items-center justify-between">
          <div className="flex items-center gap-3">
             ...
             <h3 className="text-xl font-black text-slate-800 tracking-tight">Trình phê duyệt</h3>
          </div>
        ...
        <div className="p-10 space-y-8">
          ...
          <div className="space-y-2">
            ...
            <div className="relative group">
               <select 
                 ...
                 onChange={(e) => setSubmitData(p => ({...p, receiverId: e.target.value}))}
               ...
               <div className="absolute right-6 top-1/2 -translate-y-1/2 pointer-events-none text-slate-400">
                  <FaSpinner className={registration.loading ? 'animate-spin' : 'hidden'} />
               </div>
            </div>
          </div>
          ...
          {selectedManager && (
            <div className="p-6 bg-blue-50 rounded-2xl border border-blue-100 animate-fadeIn">
               <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest">Chức vụ lãnh đạo</p>
               <p className="text-sm font-bold text-slate-700 mt-1">{selectedManager.role}</p>
            </div>
          )}
        ...
        <div className="px-10 py-8 bg-slate-50/50 border-t flex justify-end gap-4">
           <button onClick={onClose} className="px-6 py-3 font-black text-slate-400 hover:text-slate-600 text-[10px] uppercase tracking-widest transition-all">Hủy bỏ</button>
           <button 
           ...
           >
              {registration.loading ? <FaSpinner className="animate-spin" /> : <FaPaperPlane />} XÁC NHẬN GỬI
           </button>
        </div>
      </div>
    </div>
