...
import { 
  fetchPendingRequest, fetchPendingSuccess, fetchPendingFailure,
  fetchApprovalDetailRequest, fetchApprovalDetailSuccess,
  processApprovalRequest, processApprovalSuccess, processApprovalFailure
...
function* handleFetchDetail(action) {
  try {
    ...
    // Chọn API detail dựa trên loại form
    switch(formType) {
        case 'REGISTRATION': detailApi = api.getRegistrationDetailApi; break;
        case 'SALARY': detailApi = api.getSalaryDetailApi; break;
        case 'PROMOTION': detailApi = api.getPromotionDetailApi; break;
        case 'PROPOSAL': detailApi = api.getProposalDetailApi; break;
        case 'TERMINATION': detailApi = api.getTerminationDetailApi; break;
        default: detailApi = api.getRegistrationDetailApi;
    }
    ...
    yield put(fetchApprovalDetailSuccess({
...
function* handleProcess(action) {
  try {
    const { formType, formId, action: statusAction, leaderNote, note, actionDate } = action.payload;
